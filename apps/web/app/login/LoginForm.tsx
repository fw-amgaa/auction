...
import { RegistrationClosedToast } from "@/components/RegisterLink";
...
export function LoginForm({
  ...
  /** true when /register bounced the visitor here because registration is closed */
  registrationClosedNotice: boolean;
}) {
  const [state, formAction, pending] = useActionState<LoginState, FormData>(loginAction, {});
  const [showPass, setShowPass] = useState(false);
  const [closedToast, setClosedToast] = useState(registrationClosedNotice);
  ...
  return (
    <div className="flex min-h-screen text-ink-strong">
      ...
      <RegistrationClosedToast show={closedToast} onClose={() => setClosedToast(false)} />
    </div>
  );
}
